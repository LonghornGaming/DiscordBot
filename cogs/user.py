...
class User(commands.Cog):
    ...
    def getJoinInfo(self, ctx) -> str:
        joins = tuple(sorted(ctx.guild.members, key=operator.attrgetter("joined_at")))
        join_number = '?'
    ...
    async def sendProfile(self, ctx):
        embed = discord.Embed(title="Profile Info!", colour=discord.Colour(0xBF5700), description=f"Showing info for {ctx.author.mention}", timestamp=datetime.datetime.now())
        embed.set_author(name="Bevo Bot")
        embed.set_footer(text="Bevo Bot")

        embed.add_field(name=":calendar_spiral: Join Info: ", value=self.getJoinInfo(ctx))
        embed.add_field(name=":bar_chart: XP Info: ", value="XP currently being reworked\n")
